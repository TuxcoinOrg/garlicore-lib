Contributing to Tuxcore
=======

We're working hard to make *tuxcore* the most powerful JavaScript library for working with Tuxcoin. Our goal is to have *tuxcore* be a library that can be used by anyone interested in tuxcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in tuxcore that any change must adhere.
...
When possible, tuxcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of tuxcore methods. For example, calling `JSON.stringify` on a tuxcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'tuxcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the tuxcore object.

DO:
```javascript
var tuxcore = require('../');
var PublicKey = tuxcore.PublicKey;
```
DON'T:
```javascript
